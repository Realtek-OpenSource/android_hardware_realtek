...
void RGBAToNV12Row_C(const uint8* src_rgba, int src_stride_rgba,
                   uint8* dst_y, uint8* dst_y_next, uint8* dst_uv, int width) {
    int i, j;
    const uint8* src_rgba_next = (const uint8*)(src_rgba + src_stride_rgba);
    int32 pR[4], pG[4], pB[4];
    ...
    for (i = 0; i < width; i+=2)
    {
        ...
        {
            pR[j] = src_rgba[(i+j)*4];
            pG[j] = src_rgba[(i+j)*4+1];
            pB[j] = src_rgba[(i+j)*4+2];
            pR[j+2] = src_rgba_next[(i+j)*4];
            pG[j+2] = src_rgba_next[(i+j)*4+1];
            pB[j+2] = src_rgba_next[(i+j)*4+2];

            avgR += (pR[j]+pR[j+2]);
            avgG += (pG[j]+pG[j+2]);
            avgB += (pB[j]+pB[j+2]);

            dst_y[i+j] = RGB2Y(pR[j], pG[j], pB[j]);
            dst_y_next[i+j] = RGB2Y(pR[j+2], pG[j+2], pB[j+2]);
        }
        avgR /= 4;
        avgG /= 4;
        avgB /= 4;
        dst_uv[i] = RGB2U(avgR, avgG, avgB);
        dst_uv[i+1] = RGB2V(avgR, avgG, avgB);
    }
}

void RGBAToNV21Row_C(const uint8* src_rgba, int src_stride_rgba,
    ...
    for (i = 0; i < width; i+=2)
    {
        ...
        avgG /= 4;
        avgB /= 4;
        dst_vu[i] = RGB2V(avgR, avgG, avgB);
        dst_vu[i+1] = RGB2U(avgR, avgG, avgB);
    }
}
