...
int vdi_wait_interrupt(unsigned long coreIdx, unsigned int instIdx, int timeout, unsigned int addr_bit_int_reason);
#else
...
int vdi_enable_ve_prot_mode(Uint32 core_idx);
unsigned int vdi_set_dovi_flag(unsigned long core_idx, unsigned long inst_idx, unsigned int enable);
...
void vdi_write_dcsys_register(unsigned long core_idx, unsigned int addr, unsigned int data);
